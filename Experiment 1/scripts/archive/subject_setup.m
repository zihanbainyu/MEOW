...
% description:
% creats a counterbalanced, randomized trial schedule for a 2-back memory
% task with target and lures; ensure propoer spacing and response balance
% across encoding and test phases
%==========================================================================
...
% define # of stimulus pair in each condition
p.nComparison = 40;     % pairs shown together during encoding
p.nIsolated_Both = 40;  % pairs shown separately during encoding
p.nNovel = 40;          % pairs never shown during encoding, i.e., test only

% define # of test trials requiring each response type
p.nTest_Lure_Events = 120;
p.nTest_Repeat_Events = 120; 

% define keyboard response mappings
p.keys.same = 'j'; 
p.keys.diff = 'k'; 
p.keys.quit = 'escape';
...
p.timing.fix_dur = 0.75;

%% ========================================================================
%  LOAD STIMULUS FILES
%  ========================================================================
if ~exist(p.setup_dir, 'dir')
    mkdir(p.setup_dir);
end
...
%% ========================================================================
% COUNTERBALANCE TARGET-LURE ASSIGNMENT
%  ========================================================================
% For each pair, randomly decide which image is "target" and which is "lure"
% ensure no systematic bias across subjects

fprintf('assigning stimuli to conditions for subject %03d...\n');

% randomly shuffle the order of all pairs
shuffled_pairs = master_pair_list(randperm(height(master_pair_list)), :);

% create empty table to hold final assignments
final_pairs = table('Size', size(shuffled_pairs), ...
    'VariableTypes', {'string', 'string'}, ...
    'VariableNames', {'Target', 'Lure'});

% for each pair, flip a coin to decide target vs lure assignment
...
% randomly shuffle all foil images
all_foils = all_foils(randperm(numel(all_foils)));

% divide pairs into three experimental conditions
idx_start=1; 
...
% store the assignments in parameters structuree
...
%% ========================================================================
% BUILD PHASE I ENCODING SCHEDULE
%  ========================================================================
fprintf('building phase I...\n');
...
% isolated encoding events (80 blocks)
    % each stimulus appears separately with a filler image
    % therefore, need 80 foils (2 per pair: one for target block, one for lure block)
iso_event_foils = all_foils(1:2*p.nIsolated_Both); 
all_foils(1:2*p.nIsolated_Both) = []; % remove used foils

for i=1:p.nIsolated_Both
    % isolated target blocks
    encoding_blocks{end+1} = {...
        iso_event_foils(1),"iso_both",  "filler","new","none"; ...
        iso_both_pairs.Target(i),"iso_both",  "target","new","none"};
    iso_event_foils(1)=[]; % remove used foils

    % isolated lure blocks
    encoding_blocks{end+1} = {...
        iso_event_foils(1),"iso_both",  "filler","new","none"; ...
        iso_both_pairs.Lure(i),"iso_both", "lure","new","none"};   
    iso_event_foils(1)=[]; % remove used foils
end 

% create spacer blocks to match # of encoding blocks
% 40 repeat blocks provide 40 "repeat" responses
n_foil_repeats = 40;
repeat_foils = all_foils(1:n_foil_repeats); 
all_foils(1:n_foil_repeats) = []; % remove used foils


for i=1:n_foil_repeat
    % repeat foil blocks
    spacer_blocks{end+1} = { ...
...
% create remaining space with new foils
...
assert(numel(spacer_blocks)==expected_blocks, ...
    sprintf('Block count mismatch! encoding=%d, spacer=%d',...
    numel(encoding_blocks), numel(spacer_blocks))); % verify
...
encoding_schedule = cell2table(final_encoding_list, ...
...
% count how many of response type
...
fprintf('  Response balance: %d Same(j), %d Diff(k), %d None\n', ...
    p.counts.encoding.j_presses, p.counts.encoding.k_presses, ...
    p.counts.encoding.no_response);

%% ========================================================================
%  ENSURE TARGETS ENCODED BEFORE LURES
%  ========================================================================
% target must always be seen before its paired lure
% swaps trial positions if lure comes first

fprintf('\nchecking encoding order for potential target-lure reversals...\n');

% combine all pairs and condition labels
all_pair_structs = [...
    table(p.stim.comparison.Target, p.stim.comparison.Lure, ...
        repmat("comparison", p.nComparison, 1), ...
          'VariableNames', {'Target','Lure','Condition'}); ...
    table(p.stim.iso_both.Target, p.stim.iso_both.Lure,...
        repmat("iso_both", p.nIsolated_Both, 1), ...
          'VariableNames', {'Target','Lure','Condition'}); ...
    table(p.stim.novel.Target, p.stim.novel.Lure, ...
        repmat("novel", p.nNovel, 1), ...
          'VariableNames', {'Target','Lure','Condition'})];
...
for i = 1:height(all_pair_structs)
    ...
    % find where target appaers in encoding schedule
