...
function main()
    ...
    try
        ...
        p.keys.device = 8; %(magic keyboard in room 862)
        ...
        if p.eyetracking == 1
            ...
            Eyelink('command', 'calibration_type = HV13'); % use HV9 if you want better stability. in my case, precision is what i need
           ...
           % --- set up calibration ---
            el.backgroundcolour = p.colors.white / 2;
        ...
        for b = block_sequence %
            ...
            if p.eyetracking == 1
                ...
                Eyelink('CloseFile');
                try
                    status=Eyelink('ReceiveFile', edf_filename, p.results_dir, 1);
...
function mid_rest(p, el, current_block)
    ...
    DrawFormattedText(p.window, sprintf(['Wonderful! You have completed part 1 of block %d.\n\n' ...
...
function end_rest(p, current_block, total_blocks)
    rest_text = sprintf(['Great job! You have completed this block.\n\n' ...
    ...
    Screen('Flip', p.window); 
    WaitSecs(10); % This is 10s, not 120s. Adjust if needed.
