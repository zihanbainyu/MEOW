...
%% block exclusion
%%%%%%%%%%%%%%%%%%%%%%%
% block 3 is dropped here: gate_timeout hit 30.6% (1-back) / 68.4% (2-back)
% while fix_broken stayed at 0%, which points at the tracker (invalid samples
% or 100-150 px calibration drift) rather than at the subject. 2-back AB
% accuracy also fell to chance in that block. see the EDFs before deciding
% whether this exclusion is the right call.
% NOTE: results_recognition has no block column -- it is a single post-task
% run over items from all four blocks, so section 3 below is UNCHANGED from
% the full analysis and its "compared/isolated/novel" cells still contain
% block-3 items.
exclude_blocks = 3;
n1_before = height(r1); n2_before = height(r2);
r1(ismember(r1.block, exclude_blocks), :) = [];
r2(ismember(r2.block, exclude_blocks), :) = [];

diary_file = fullfile(res_dir, sprintf('sub%03d_behavioral_report_noblk3.txt', subj_id));
if exist(diary_file,'file'), delete(diary_file); end
diary(diary_file);
fprintf('================================================================\n');
fprintf(' SINGLE-SUBJECT BEHAVIORAL REPORT (BLOCK 3 EXCLUDED)  --  sub%03d   (%s)\n', subj_id, datestr(now,'yyyy-mm-dd HH:MM'));
fprintf('================================================================\n');
fprintf('excluded block(s) [%s]: 1-back %d -> %d trials, 2-back %d -> %d trials\n', ...
    num2str(exclude_blocks), n1_before, height(r1), n2_before, height(r2));
fprintf('recognition is a single post-task run with no block column, so section 3 is unfiltered.\n');
...
save(fullfile(res_dir, sprintf('sub%03d_indiv_stats_noblk3.mat', subj_id)), 'indiv');
fprintf('\n\nsaved: %s\n', fullfile(res_dir, sprintf('sub%03d_indiv_stats_noblk3.mat', subj_id)));
fprintf('saved: %s\n', diary_file);
diary off;
...
set(gca,'XTick',blocks); box off; hold off;
save_fig(f, fig_dir, sprintf('sub%03d_qc_noblk3', subj_id));
...
save_fig(f, fig_dir, sprintf('sub%03d_1back_noblk3', subj_id));
...
xtickangle(30);
save_fig(f, fig_dir, sprintf('sub%03d_2back_noblk3', subj_id));
...
save_fig(f, fig_dir, sprintf('sub%03d_confusion_noblk3', subj_id));
...
if has_rec && ~isempty(rec_d)
    ...
    save_fig(f, fig_dir, sprintf('sub%03d_recognition_noblk3', subj_id));
end
