...
try
    ...
    %% ========================================================================
    %   MAIN EXPERIMENT LOOP (PHASE 1: N-BACK TASKS)
    %  ========================================================================
    % --- global instructions ---
    ...
    % Define which block to run: all or some?
    b_to_run = 0; if b_to_run == 0, b_seq = 1:p.nBlocks; else, b_seq = b_to_run; end
    for b = b_seq
        ...
        if p.eyetracking == 1
            ...
            results_table = C_run_1_back(p, el, sequence_1_back_block);
        ...
        if p.eyetracking == 1
            ...
            % Eye-tracking version
            results_table = D_run_2_back(p, el, sequence_2_back_block);
            fprintf('EYELINK: receiving edf file: %s\n', edf_filename);
            Eyelink('CloseFile');
            try
                Eyelink('ReceiveFile', edf_filename, p.results_dir, 1);
    ...
    % --- Save the consolidated file (Part 1: N-Back data) ---
    save(final_data_filename, 'final_data_output');
    fprintf('All N-Back data saved to:\n%s\n', final_data_filename);
    ...
    if p.eyetracking == 1
            ...
            % Eye-tracking version
            results_table = E_run_recognition(p, el, sequence_recognition); % This returns the rec data
            fprintf('EYELINK: receiving edf file: %s\n', edf_filename);
            Eyelink('CloseFile');
            try
                Eyelink('ReceiveFile', edf_filename, p.results_dir, 1);
    ...
    % --- [FIX] Add recognition data to final consolidated file and re-save ---
    final_data_output.results_recognition = results_table;
    save(final_data_filename, 'final_data_output');
    fprintf('All data (N-Back + Rec) saved to:\n%s\n', final_data_filename);
