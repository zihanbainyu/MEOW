...
raincloud(data, {c_same, c_sim, c_new}, {'targ det','lure disc','diff rej'}, 'Accuracy', '', [0,1]);
add_sig(data, [1 2; 2 3; 1 3]);
subplot(1,3,2);
data = [b1_rt_sam, b1_rt_sim];
raincloud(data, {c_same, c_sim}, {'targ det','lure disc'}, 'RT (s)', '');
add_sig(data, [1 2]);
...
raincloud(mat_comp, {c_comp, c_comp*0.5+[1 1 1]*0.5}, {'correct', 'incorrect'}, 'Gaze reinstatement', 'compared B2B1', []);
add_sig(mat_comp, [1 2]);
subplot(1,2,2);
mat_iso = [gaze_corr(:,2), gaze_incorr(:,2)];
raincloud(mat_iso, {c_iso, c_iso*0.5+[1 1 1]*0.5}, {'correct', 'incorrect'}, 'Gaze reinstatement', 'isolated B2B1', []);
add_sig(mat_iso, [1 2]);
set(gcf, 'PaperPositionMode', 'auto');
print(gcf, fullfile(fig_dir, 'gaze_bb_correctness.pdf'), '-dpdf', '-vector');
...
% % use raw p value for post hoc tests
