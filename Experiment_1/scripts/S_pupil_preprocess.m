...
for r = 1:4
    ...
    % 3. dva conversion & blink interp
    [x_d, y_d] = px2dva(raw.xp, raw.yp, C);
    in.xPos{r} = x_d; in.yPos{r} = y_d; 
    in.pupilArea{r} = fillmissing(raw.pp, 'linear'); % required for PCDM filtering
    in.sampleRate{r} = C.SR;
    ...
    save(fullfile(r_dir, sprintf('sub%03d_2_%01d_input_v3.mat', subj_id, r)), 'in');
    fprintf('done (%d trials).\n', sum(~bad));
end

% 6. GET CLEAN DATA (No Fitting)
fprintf('running dataAnalysis (preprocessing only)...\n');
addpath(genpath(fullfile(base_dir, 'scripts', 'PCDM-main'))); 
d = dataAnalysis(in); % <--- REPLACED fitModel WITH dataAnalysis
save(fullfile(r_dir, sprintf('sub%03d_clean_pupil.mat', subj_id)), 'd');
fprintf('--- all done ---\n');
...
function r = parse_asc(f)
    ...
    while ~feof(fid)
        ...
        if l(1)>='0' && l(1)<='9' 
            ...
            else, ts(c)=sscanf(l,'%d',1); end 
        elseif startsWith(l, 'MSG') 
            ec=ec+1; dat = textscan(l, 'MSG %d %s', 'Delimiter', ''); 
