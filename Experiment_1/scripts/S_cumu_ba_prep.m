...
fprintf('Matched B-A pairs: compared=%d, isolated=%d\n', height(pairs_ba_comp), height(pairs_ba_iso));

unique_subjs = unique([pairs_ba_comp.subj_id; pairs_ba_iso.subj_id]);
...
for s_idx = 1:length(unique_subjs)
    ...
    subj_comp = pairs_ba_comp(pairs_ba_comp.subj_id == sid, :);
    subj_iso = pairs_ba_iso(pairs_ba_iso.subj_id == sid, :);
    if height(subj_comp) > 0
        subj_baseline_b_comp.(skey) = tr_one_b_b_comp(tr_one_b_b_comp.subj_id == sid & ismember(tr_one_b_b_comp.trial_id, subj_comp.tr_1b_b), :);
    end
    if height(subj_iso) > 0
        subj_baseline_b_iso.(skey) = tr_one_b_b_iso(tr_one_b_b_iso.subj_id == sid & ismember(tr_one_b_b_iso.trial_id, subj_iso.tr_1b_b), :);
    end
end

max_fixations = 10;

fprintf('\nComputing cumulative B-A compared (%d pairs)...\n', height(pairs_ba_comp));
cumulative_results_comp = nan(height(pairs_ba_comp), max_fixations + 3);
n_comp = 0;
for i = 1:height(pairs_ba_comp)
    ...
    n_comp = n_comp + 1;
    cumulative_results_comp(n_comp, :) = [pair.subj_id, pair.tr_1b_b, pair.tr_2b_a, cumulative_reinst];
end
cumulative_results_comp = cumulative_results_comp(1:n_comp, :);

fprintf('Computing cumulative B-A isolated (%d pairs)...\n', height(pairs_ba_iso));
cumulative_results_iso = nan(height(pairs_ba_iso), max_fixations + 3);
n_iso = 0;
for i = 1:height(pairs_ba_iso)
    ...
    n_iso = n_iso + 1;
    cumulative_results_iso(n_iso, :) = [pair.subj_id, pair.tr_1b_b, pair.tr_2b_a, cumulative_reinst];
end
cumulative_results_iso = cumulative_results_iso(1:n_iso, :);

fprintf('Cumulative B-A: compared=%d trials, isolated=%d trials\n', n_comp, n_iso);

%% Aggregate and plot
fixation_names = arrayfun(@(x) sprintf('fix_%d', x), 1:max_fixations, 'UniformOutput', false);
var_names = [{'subj_id', 'tr_1b_b', 'tr_2b_a'}, fixation_names];
...
ylabel('Cross-Item Reinstatement (baseline-corrected)', 'FontSize', 12);
title('Fixation-by-Fixation B\rightarrowA Cross-Item Reinstatement', 'FontSize', 14);
legend({'', '', 'Compared', 'Isolated'}, 'Location', 'best', 'FontSize', 11);
xlim([0.5, n_fix_to_plot + 0.5]); box off; grid on;
print(gcf, fullfile(out_dir, 'cumulative_reinstatement_ba.pdf'), '-dpdf', '-vector');

save(fullfile(out_dir, 'cumu_reinstat_ba.mat'), ...
