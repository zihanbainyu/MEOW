...
% directory
base_dir = '..';
p.stim_dir = fullfile(base_dir, 'stimulus/stim/');
...
fprintf('Found %d L1 pairs, %d L2 pairs, and %d foils.\n', ...
        height(master_pair_list_l1), height(master_pair_list_l2), numel(all_foils));
...
% =====================================================================
   ...
   % =====================================================================
    ...
    % We only need ~20 repeats. 30 is too many for our foil budget.
    n_foil_repeats = 30; 
    ...
    % --- combine ALL blocks and shuffle them together ---
    % We have 30 comp blocks + 20 repeat blocks + 60 iso singles = 110 blocks
    all_encoding_blocks = [comp_miniblocks, repeat_miniblocks, iso_single_trials];
    ...
    % Concatenate the blocks into one long trial list
    % Total trials: (30*2) + (20*2) + (60*1) = 160 trials.
    final_encoding_list = vertcat(final_ordered_blocks{:});
    ...
    % =====================================================================
    %
    % 5D: BUILD TEST PHASE FOR THIS BLOCK (STATE-BASED GENERATOR - FIXED)
    %
    % =====================================================================
    % =====================================================================
    ...
    if isempty(all_foils), error('Ran out of foils!'), end;
    junk1 = all_foils(1);
    all_foils(1) = [];
    if isempty(all_foils), error('Ran out of foils!'), end;
    junk2 = all_foils(1);
    ...
    stream_items = {char(junk1), char(junk2)}; % Start with 2 junk foils
    stream_responses = {"none", "none"};
    ...
    while goal_idx <= height(goal_list) || ~isempty(current_goal)
        ...
        if ~isempty(current_goal) && string(N2_item) == string(current_goal.O_item) && (startsWith(N1_item, "mst_") && endsWith(N1_item, "_foil.png"))
...
% This function must be at the end of the script
function [foil, stack] = get_new_foil(stack)
    if isempty(stack)
