...
fprintf('A2-A1 isolated: %d pairs\n', height(pairs_aa_iso));
fprintf('A2-B1 isolated: %d pairs\n', height(pairs_ab_iso));

unique_subjs = unique([pairs_aa_comp.subj_id; pairs_aa_iso.subj_id; pairs_ab_iso.subj_id]);
n_subjs = length(unique_subjs);

subj_pool_b_comp = struct();
subj_pool_b_iso = struct();
for s = 1:n_subjs
    sid = unique_subjs(s); skey = sprintf('s%d', sid);
    sb_comp = tr_one_b_b_comp(tr_one_b_b_comp.subj_id == sid, :);
    sb_iso = tr_one_b_b_iso(tr_one_b_b_iso.subj_id == sid, :);
    if height(sb_comp) > 0, subj_pool_b_comp.(skey) = sb_comp; end
    if height(sb_iso) > 0, subj_pool_b_iso.(skey) = sb_iso; end
end

gaze_aa_comp = nan(n_subjs, 1);
gaze_aa_iso = nan(n_subjs, 1);
gaze_ab_iso = nan(n_subjs, 1);

for s = 1:n_subjs
    sid = unique_subjs(s); skey = sprintf('s%d', sid);

    sp = pairs_aa_comp(pairs_aa_comp.subj_id == sid, :);
    if height(sp) > 0 && isfield(subj_pool_b_comp, skey)
        pool = subj_pool_b_comp.(skey);
        r_vals = nan(height(sp), 1);
        for i = 1:height(sp)
            fix_2b = Mw(task_2b_idx & Mw.subj_id == sid & Mw.trial_id == sp.tr_2b_a(i), :);
            if height(fix_2b) < 2, continue; end
            map_2b = create_fixation_map(fix_2b.x, fix_2b.y, fix_2b.dur, spatial_params);
            if sum(map_2b(:)) == 0, continue; end
            fix_1b_match = Mw(task_1b_idx & Mw.subj_id == sid & Mw.trial_id == sp.tr_1b_a(i), :);
            if height(fix_1b_match) < 2, continue; end
            map_match = create_fixation_map(fix_1b_match.x, fix_1b_match.y, fix_1b_match.dur, spatial_params);
            if sum(map_match(:)) == 0, continue; end
            r_match = corr(map_2b(:), map_match(:));
            partner_b = regexprep(sp.base_id{i}, '_BASE_', '_B_');
            bp = pool(~strcmp(pool.stim_id, partner_b), :);
            if height(bp) < 1, continue; end
            mm = nan(height(bp), 1);
            for j = 1:height(bp)
                fix_mm = Mw(task_1b_idx & Mw.subj_id == sid & Mw.trial_id == bp.trial_id(j), :);
                if height(fix_mm) < 2, continue; end
                map_mm = create_fixation_map(fix_mm.x, fix_mm.y, fix_mm.dur, spatial_params);
                if sum(map_mm(:)) > 0, mm(j) = corr(map_2b(:), map_mm(:)); end
            end
            r_vals(i) = r_match - mean(mm, 'omitnan');
        end
        gaze_aa_comp(s) = mean(r_vals, 'omitnan');
    end

    sp = pairs_aa_iso(pairs_aa_iso.subj_id == sid, :);
    if height(sp) > 0 && isfield(subj_pool_b_iso, skey)
        ...
        gaze_aa_iso(s) = mean(r_vals, 'omitnan');
    end
    ...
    if height(sp) > 0 && isfield(subj_pool_b_iso, skey)
        ...
        for i = 1:height(sp)
            ...
            map_2b = create_fixation_map(fix_2b.x, fix_2b.y, fix_2b.dur, spatial_params);
            if sum(map_2b(:)) == 0, continue; end
            fix_1b_match = Mw(task_1b_idx & Mw.subj_id == sid & Mw.trial_id == sp.tr_1b_b(i), :);
            if height(fix_1b_match) < 2, continue; end
            map_match = create_fixation_map(fix_1b_match.x, fix_1b_match.y, fix_1b_match.dur, spatial_params);
            if sum(map_match(:)) == 0, continue; end
            r_match = corr(map_2b(:), map_match(:));
        ...
        gaze_ab_iso(s) = mean(r_vals, 'omitnan');
    end
