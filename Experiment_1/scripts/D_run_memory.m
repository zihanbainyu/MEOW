...
function [results_table] = D_run_memory(p, el, test_schedule_block)
    ...
    block_start_text = ['Upcoming: Part B (2-Back)\n\n', ...
        ...
        '\n\n', ...
        'Reminder, you are comparing the current image to the one from TWO TRIALS AGO.\n\n\n', ...
    ...
    for i = 1:height(results_table)
        ...
        if is_eyetracking
            ...
            % send variables to Data Viewer for analysis (Applied char() fix)
            ...
            Eyelink('Message', '!V TRIAL_VAR role %s', char(trial_info.role));
            Eyelink('Message', '!V TRIAL_VAR trial_type %s', char(trial_info.trial_type_final));
        ...
        % Recalculate and store accuracy (outside of Eyelink block for non-ET)
        if ~is_eyetracking
            results_table.correct(i) = calculate_accuracy(key_pressed, string(trial_info.correct_response));
        end
