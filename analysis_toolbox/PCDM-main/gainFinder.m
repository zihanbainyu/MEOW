...
elseif op.fitTimeseries == 0
    ...
    for kk = 1:numTrialTypes
        ...
        minLen = min(length(pupilAvg(kk,:)), length(predT2(kk,:)));
        pupilAvg_matched = pupilAvg(kk, 1:minLen);
        predT2_matched = predT2(kk, 1:minLen);
        DM = [ones(minLen,1), predT2_matched'];
        sol = regress(pupilAvg_matched', DM);
