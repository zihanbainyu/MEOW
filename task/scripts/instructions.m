...
function instructions(p, instruction_type)
    ...
    switch instruction_type
        ...
        DrawFormattedText(p.window, ['Welcome to our study!\n\n' ...
            '  \n\n' ...
            'Today, you will view a series of items and make responses across two phases,\n\n' ...
        ...
        case 'encoding'
            ...
            DrawFormattedText(p.window, ['You have three possible responses:\n\n' ...
                '  \n\n' ...
                'If the object is exactly the same as the previous one, press "', upper(p.keys.same), '" (Repeat).\n\n' ...
                '  \n\n' ...
                'If the object is similar but not identical, press "', upper(p.keys.diff), '" (Similar).\n\n' ...
                '  \n\n' ...
                'If the object is new, do not press any key. Just wait for the next item.\n\n'], ...
                'center', 'center', p.colors.black, [], [], [], line_spacing);
            Screen('Flip', p.window);
            waitForKeyPress(space_key, escape_key);
            
            try
                img_repeat = imread(fullfile(p.stim_dir, 'example_target.png'));
            ...
            DrawFormattedText(p.window, 'First, a fixation cross will appear at the center of the screen:', ...
                'center', p.yCenter - 250, p.colors.black, [], [], [], line_spacing);
            DrawFormattedText(p.window, '+', 'center', 'center', p.colors.black); 
            Screen('Flip', p.window);
            waitForKeyPress(space_key, escape_key);

            DrawFormattedText(p.window, 'Next, you might see an item like this:', 'center', p.yCenter - 250, p.colors.black, [], [], [], line_spacing);
            Screen('DrawTexture', p.window, tex_repeat, [], [], 0);  % exact real task location
            Screen('Flip', p.window);
            waitForKeyPress(space_key, escape_key);
            ...
            DrawFormattedText(p.window, ['You should say: "Repeat"  (press "', upper(p.keys.same), '")'], ...
            ...
            DrawFormattedText(p.window, ['You should say: "Similar"  (press "', upper(p.keys.diff), '")'], ...
                'center', p.yCenter + 250, p.colors.black);
            ...
            DrawFormattedText(p.window, 'Here, if the next item looks like this:', ...
                'center', p.yCenter - 250, p.colors.black, [], [], [], line_spacing);
            Screen('DrawTexture', p.window, tex_new, [], [], 0);
            DrawFormattedText(p.window, 'You should not press any key.', ...
                'center', p.yCenter + 250, p.colors.black);
