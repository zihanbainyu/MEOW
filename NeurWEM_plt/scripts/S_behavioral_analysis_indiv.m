...
% condition set for this experiment (order fixes colours + reporting)
cnd_nm   = {'compared','novel'};
cnd_cols = {c_comp,    c_nov};
nC       = numel(cnd_nm);
...
fprintf(' Exp3_CompNov  (behaviour only, conditions: %s)\n', strjoin(cnd_nm, ' / '));
fprintf('================================================================\n');
...
%% 1. encoding (1-back)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1-back structure (Exp3): compared mini-block {A none, B k} + repeat/foil
% mini-block {A none, A j}. No isolated condition.
v1     = r1.rt > min_rt;
...
i1_new = (r1.condition=="compared" & r1.identity=="A") | ...
...
if nC > 2, fprintf('    -> %s\n', kw_str(rt_l)); else, rank_test('    -> compared vs novel', rt_l{1}, rt_l{2}); end
fprintf('  target (AA):\n'); for c=1:nC, rt_report(sprintf('  %-9s', cnd_nm{c}), rt_t{c}); end
if nC > 2, fprintf('    -> %s\n', kw_str(rt_t)); else, rank_test('    -> compared vs novel', rt_t{1}, rt_t{2}); end
...
% visualization
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% fig 1: data quality / time on task  (no gaze QC in Exp3)
...
if has_rec && ~isempty(rec_d)
    ...
    cmap = containers.Map({'compared','novel','foil'}, {c_comp, c_nov, c_new});
