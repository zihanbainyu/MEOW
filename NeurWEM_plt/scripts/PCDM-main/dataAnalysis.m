...
op.downsampleRate = 10; % downsample rate for the deconvolution design matrix. Higher numbers makes code run faster, but shouldn't be set higher than Nyquist frequency.
...
in.predictionWindow = 1.5; % The time window you want to make the prediction for the TEPR, beyond the trial onset time (in seconds). Usually the experimenter-set trial duration (if you're using fixed trial durations), but can be shorter if you want.
...
for ff = 1:numRuns
    ...
    if op.interpolateBlinks == 1
        ...
        if any(~isfinite(in.pupilArea2))
            warning('Blink interpolation produced non-finite values. Using fillmissing instead.');
            in.pupilArea2(~isfinite(in.pupilArea2)) = NaN;
            in.pupilArea2 = fillmissing(in.pupilArea2, 'linear', 'EndValues', 'nearest');
        end
    end
