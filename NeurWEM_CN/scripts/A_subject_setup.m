...
% (Run durations depend on the ITI and on the 1-back fillers; see
% p.nFiller_1back and p.timing below.)
p.nBlocks = 4;

% Single-presentation filler items per 1-back run. Fillers are foils shown
% once (corr_resp "none") that space the compared pairs apart; the 1-back
% sequence is built so no two compared pairs are adjacent (see P5A).
p.nFiller_1back = 30;
...
p.keys.diff = '2';
p.keys.quit = 'escape';
p.keys.trigger = '5%';   % scanner trigger ("5" marks scan onset); used by the fMRI run scripts
...
p.timing.block_lead_in = 6;         % seconds before the first trial of a run

% Fixation-only tail after the last trial of the 1-back and 2-back runs,
% mirroring the lead-in so the final trial's response is fully captured
% before the run ends. Honoured by C_run_1_back.m and D_run_2_back.m.
% (Recognition has neither lead-in nor tail.)
p.timing.block_tail = 6;

% Mid-run fixation break inserted into each 1-back run, roughly (not exactly)
% at the midpoint and always at a miniblock boundary so it never splits a
% pair. Its per-block position is drawn in P5A and stored in
% p.timing.midfix_after_trial_1back. Honoured by C_run_1_back.m.
p.timing.block_midfix = 6;
...
% --- Condition pools come from L2 (bin 2) and L3 (bin 3), matching NeurWEM ---
%     (here master_pair_list_l1 = the bin-2 level, master_pair_list_l2 = bin-3).
...
% --- Repeat pairs come from L4 (bin 4): A is shown A-A in the 1-back; its B is
%     never shown until it becomes the MST lure (P6B). ---
...
% Load foils (A images only, shown once as new/filler items)
...
fprintf('Found %d L2 pairs, %d L3 pairs, %d repeat(l4) pairs, %d foils.\n', ...
...
% --- Partition compared/novel (per level) and repeat (single L4 pool) into blocks ---
...
fprintf('  -> Each block: %d compared, %d novel, %d repeat pairs.\n', ...
    ...
    numel(p.block_indices.repeat{1}));
%
...
for b = 1:p.nBlocks
    ...
    % Foils shown once, no repeat, corr_resp "none". They pad the run so the
    % compared pairs can be held apart in the interleave below.
    filler_miniblocks = {};
    n_filler = p.nFiller_1back;
    if height(all_foils_remain) < n_filler
        error('Not enough foils for 1-back fillers in Block %d', b);
    end
    filler_foil_items = all_foils_remain(1:n_filler, :);
    all_foils_remain(1:n_filler, :) = [];
    for i = 1:n_filler
        filler_miniblocks{end+1} = { ...
            filler_foil_items.A_foil(i), "filler", "F", "none"};
    end

    % ---- Interleave so no two compared pairs are adjacent ----------------
    % Compared miniblocks are dropped into the stream of non-compared units
    % (repeats + fillers), at most one per gap, so every pair of compared
    % trials is separated by at least one non-compared trial. Which gaps are
    % used is random, so the compared train stays aperiodic.
    noncomp_miniblocks = [repeat_miniblocks, filler_miniblocks];
    noncomp_miniblocks = noncomp_miniblocks(randperm(numel(noncomp_miniblocks)));
    comp_shuffled = comp_miniblocks(randperm(numel(comp_miniblocks)));

    nNon = numel(noncomp_miniblocks);
    nCompMB = numel(comp_shuffled);
    gaps = randperm(nNon + 1, nCompMB) - 1;   % distinct gaps in 0..nNon

    final_miniblocks = {};
    ci = 1;
    for k = 0:nNon
        if ci <= nCompMB && any(gaps == k)
            final_miniblocks{end+1} = comp_shuffled{ci};
            ci = ci + 1;
        end
        if k < nNon
            final_miniblocks{end+1} = noncomp_miniblocks{k + 1};
        end
    end
    ...
    % 10 s break never splits a pair. Stored as a within-block trial index and
    % honoured by C_run_1_back.m.
    ...
    p.counts.oneback.resp_same = p.counts.oneback.resp_same + ...
        sum(strcmp(sequence_1_back_block.corr_resp, '1'));
    p.counts.oneback.resp_similar = p.counts.oneback.resp_similar + ...
        sum(strcmp(sequence_1_back_block.corr_resp, '2'));
    ...
    p.counts.twoback.resp_same = p.counts.twoback.resp_same + ...
        sum(strcmp(sequence_2_back_block.corr_resp, '1'));
    p.counts.twoback.resp_similar = p.counts.twoback.resp_similar + ...
        sum(strcmp(sequence_2_back_block.corr_resp, '2'));
...
% MST uses the same jittered ITI as the n-back runs
