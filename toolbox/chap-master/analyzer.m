    ...
    methods(Static)
        ...
        function data = analyze_all(data, data_mat, cond_names, comp_nums, conds, events_names, comp_names, log)
            ...
            for comp = 1:size(comp_nums, 1) % comp - index for posible comparation
                ...
                valid_indices = cond_ids(cond_ids <= length(data.blinks_data));

                if length(valid_indices) < length(cond_ids)
                    warning('Some condition indices were skipped because blink data was missing for those trials.');
                end

                data.cond_blinks.(comp_name) = data.blinks_data(valid_indices);
