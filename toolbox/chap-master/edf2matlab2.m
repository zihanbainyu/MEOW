...
%     pupil_size = -diff(pupil_x);
    ...
    % Find the number of valid trials where we have both a Start and an End
    num_valid_trials = min(length(trial_data.trial_names), length(trial_data.Trial_Offset_num));
    
    % Truncate all arrays to the same length
    trial_data.trial_names     = trial_data.trial_names(1:num_valid_trials);
    trial_data.Trial_Onset_num  = trial_data.Trial_Onset_num(1:num_valid_trials);
    trial_data.Trial_Offset_num = trial_data.Trial_Offset_num(1:num_valid_trials);
    % -----------------------------
