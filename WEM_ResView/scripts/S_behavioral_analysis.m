...
% [stat_results.gaze_ba_2x2, stat_results.gaze_ba_2x2_ph] = run_2x2_anova_correctness('A2-B1', gaze_ba_corr(:,1), gaze_ba_incorr(:,1), gaze_ba_corr(:,2), gaze_ba_incorr(:,2), gaze_ba_overall(:,1), gaze_ba_overall(:,2));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% manuscript descriptives (means + SDs)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ms = @(lab, v) fprintf('%-22s mean=%.4f (%.1f%%), SD=%.4f (%.1f%%)\n', lab, mean(v,'omitnan'), 100*mean(v,'omitnan'), std(v,'omitnan'), 100*std(v,'omitnan'));

fprintf('\n=== 1-back accuracies ===\n');
ms('same',    get_v('one','acc_same'));
ms('similar', get_v('one','acc_sim'));
ms('new',     get_v('one','acc_new'));

fprintf('\n=== 1-back confusions ===\n');
ms('similar->same', get_v('one','err_sim_as_same'));
ms('similar->new',  get_v('one','err_sim_as_new'));
ms('same->similar', get_v('one','err_same_as_sim'));
ms('same->new',     get_v('one','err_same_as_new'));
ms('new->same',     get_v('one','err_new_as_same'));
ms('new->similar',  get_v('one','err_new_as_sim'));

fprintf('\n=== 2-back accuracies (avg across compared/isolated/novel) ===\n');
acc_same_2b = (get_v('two','acc_AA_comp') + get_v('two','acc_AA_iso') + get_v('two','acc_AA_nov'))/3;
acc_sim_2b  = (get_v('two','acc_AB_comp') + get_v('two','acc_AB_iso') + get_v('two','acc_AB_nov'))/3;
acc_new_2b  = (get_v('two','acc_AN_comp') + get_v('two','acc_AN_iso') + get_v('two','acc_AN_nov'))/3;
ms('same (AA)',    acc_same_2b);
ms('similar (AB)', acc_sim_2b);
ms('new (AN)',     acc_new_2b);
...
% % B2-B1 similarity - memory
% sm=[gaze_bb_corr(:,1), gaze_bb_incorr(:,1), gaze_bb_corr(:,2), gaze_bb_incorr(:,2)];
% cols={c_comp,c_comp*.5+.5,c_iso,c_iso*.5+.5}; xp=[1 2 3.5 4.5];
% yl=[-0.3 0.5];
% figure('color','w','position',[100 100 500 400]); hold on;
...
% yline(0,'r--','LineWidth',2); ylim(yl);
% st=diff(yl)*.06; lv=0;
% for g=[1 3], [~,p]=ttest(sm(:,g),sm(:,g+1),'Tail','right'); lv=lv+1; yp=yl(2)-st*(2-lv);
...
% ylabel('Gaze Consistency (r)','FontSize',20); xlim([.3 5.2]); set(gca,'YTick',[-0.3 0 0.2 0.4]); box off;
% print(gcf,fullfile(fig_dir,'gaze_b2b1_cor.pdf'),'-dpdf','-vector');



% % A2-B1 similarity - memory
% sm=[gaze_ba_corr(:,1), gaze_ba_incorr(:,1), gaze_ba_corr(:,2), gaze_ba_incorr(:,2)];
% cols={c_comp,c_comp*.5+.5,c_iso,c_iso*.5+.5}; xp=[1 2 3.5 4.5];
% yl=[-0.3 0.5];
% figure('color','w','position',[100 100 500 400]); hold on;
% jt=-0.15-rand(size(sm))*.2;
...
%     text(mean(xp(g:g+1)),yp+st*.15,tx,'HorizontalAlignment','center','FontSize',20,'FontWeight','bold'); end
% set(gca,'XTick',xp,'XTickLabel',{'Correct','Incorrect','Correct','Incorrect'},'FontSize',20);
% ylabel('Gaze Similarity (r)','FontSize',20); xlim([.3 5.2]); set(gca,'YTick',[-0.3 0 0.2 0.4]);box off;
% print(gcf,fullfile(fig_dir,'reinst_a2b1.pdf'),'-dpdf','-vector');
...
function [res, posthoc] = run_2x2_anova_correctness(lbl, cc, ci, ic, ii, cond_comp, cond_iso)
    ...
    [~,p1,~,s1] = ttest(cc, ci, 'Tail','right'); [~,p2,~,s2] = ttest(ic, ii, 'Tail','right'); [~,p3,~,s3] = ttest(cc-ci, ic-ii);
    df = s1.df;
